...
// Package main 提供股权代币化合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建股权代币化应用。
// 通过本示例，您可以学习：
//   - 如何使用 helpers/rwa 模块进行股权验证和代币化
//   - 如何使用 helpers/token 模块进行股权转移
//   - 如何使用 helpers/market 模块进行股权托管和分红释放
//   - 如何利用 ISPC 受控外部交互机制替代传统预言机
//
// 🎯 核心功能
//
//  1. TokenizeEquity - 股权代币化
//     - 通过 ISPC 受控机制调用外部验证服务（验证股权合法性）
//     - 通过 ISPC 受控机制调用外部估值服务（评估股权价值）
//     - 自动生成代币并上链
//
//  2. TransferEquity - 股权转移
//     - 使用 token.Transfer() 转移股权份额
//     - 支持部分份额转移
//
//  3. EscrowEquity - 股权托管
//     - 使用 market.Escrow() 创建股权托管
//     - 适用于股权交易、质押等场景
//
...
// EquityContract 股权代币化合约
//
// 本合约展示如何使用 SDK 基础能力构建股权代币化应用：
//   - rwa.ValidateAndTokenize() - 股权验证和代币化（内部调用 token.Mint）
//   - token.Transfer() - 股权转移
//   - market.Escrow() - 股权托管
//   - market.Release() - 分红释放
//
...
//   - SDK 提供"积木"（基础能力）
//   - 应用层用"积木"搭建"建筑"（业务场景）
//   - 本示例展示如何使用"积木"构建股权代币化应用
type EquityContract struct {
...
//
//	{
//	  "asset_id": "equity_001",           // 股权ID（必填，如公司股票代码）
...
func TokenizeAsset() uint32 {
	...
	// 步骤2：构建股权文档
	// 实际应用中，股权文档应包含完整的股权信息，如：
	//   - 公司名称和注册号
	//   - 股权类型（普通股、优先股等）
	//   - 股权比例
	//   - 法律文件哈希
	//   - 股东证明等
	documentsJSON := `{"asset_id":"` + assetID + `","type":"equity"}`
...
// TransferEquity 股权转移
//
// 转移公司股权的代币份额。支持部分份额转移，适用于股权交易、投资等场景。
//
// 参数格式（JSON）:
//
//	{
...
func TransferAsset() uint32 {
	...
	// 步骤5：发出股权转移事件
	event := framework.NewEvent("EquityTransferred")
...
func EscrowAsset() uint32 {
	...
	// 步骤5：发出股权托管事件
	event := framework.NewEvent("EquityEscrowed")
...
func ReleaseYield() uint32 {
	...
	// 步骤5：发出分红释放事件
	event := framework.NewEvent("DividendReleased")
	event.AddAddressField("from", caller)
