...
// Package main 提供债券代币化合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建债券代币化应用。
// 通过本示例，您可以学习：
//   - 如何使用 helpers/rwa 模块进行债券验证和代币化
//   - 如何使用 helpers/token 模块进行债券转移
//   - 如何使用 helpers/market 模块进行债券托管和收益释放
//   - 如何利用 ISPC 受控外部交互机制替代传统预言机
//
// 🎯 核心功能
//
//  1. TokenizeBond - 债券代币化
//     - 通过 ISPC 受控机制调用外部验证服务（验证债券合法性）
//     - 通过 ISPC 受控机制调用外部估值服务（评估债券价值）
//     - 自动生成代币并上链
//
//  2. TransferBond - 债券转移
//     - 使用 token.Transfer() 转移债券份额
//     - 支持部分份额转移
//
//  3. EscrowBond - 债券托管
//     - 使用 market.Escrow() 创建债券托管
//     - 适用于债券交易、质押等场景
//
//  4. ReleaseInterest - 利息释放
//     - 使用 market.Release() 创建分阶段利息释放
//     - 适用于债券利息分配等场景
//
// 📚 相关文档
//
...
// BondContract 债券代币化合约
//
// 本合约展示如何使用 SDK 基础能力构建债券代币化应用：
//   - rwa.ValidateAndTokenize() - 债券验证和代币化（内部调用 token.Mint）
//   - token.Transfer() - 债券转移
//   - market.Escrow() - 债券托管
//   - market.Release() - 利息释放
//
...
func Initialize() uint32 {
	...
	event.AddStringField("contract", "Bond")
...
//
//	{
//	  "asset_id": "bond_001",              // 债券ID（必填，如债券代码）
//	  "total_supply": 1000000,             // 总供应量（必填，代表债券面值总额）
//	  "token_id": "BOND_001"               // 代币ID（可选，如果不提供则由系统生成）
//	}
//
// 工作流程：
//  1. 解析参数并验证
...
func TokenizeAsset() uint32 {
	...
	//   - 法律文件哈希
	//   - 信用评级等
	documentsJSON := `{"asset_id":"` + assetID + `","type":"bond"}`
	...
	//   - 链下系统同步
	//   - 审计和追溯
	event := framework.NewEvent("BondTokenized")
	event.AddAddressField("owner", caller)
...
// TransferBond 债券转移
//
// 转移债券的代币份额。支持部分份额转移，适用于债券交易、投资等场景。
//
// 参数格式（JSON）:
//
//	{
...
func TransferAsset() uint32 {
	...
	// 步骤5：发出债券转移事件
	event := framework.NewEvent("BondTransferred")
...
func EscrowAsset() uint32 {
	...
	// 步骤5：发出债券托管事件
	event := framework.NewEvent("BondEscrowed")
...
// ReleaseInterest 利息释放
//
// 创建分阶段利息释放计划，将利息按计划分阶段释放给受益人。
// 适用于债券利息分配等场景。
//
// 参数格式（JSON）:
//
...
func ReleaseYield() uint32 {
	...
	// 步骤5：发出利息释放事件
	event := framework.NewEvent("InterestReleased")
	event.AddAddressField("from", caller)
