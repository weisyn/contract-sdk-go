...
// Package main 提供商品代币化合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建商品代币化应用。
// 通过本示例，您可以学习：
//   - 如何使用 helpers/rwa 模块进行商品验证和代币化
//   - 如何使用 helpers/token 模块进行商品转移
//   - 如何使用 helpers/market 模块进行商品托管和收益释放
//   - 如何利用 ISPC 受控外部交互机制替代传统预言机
//
// 🎯 核心功能
//
//  1. TokenizeCommodity - 商品代币化
//     - 通过 ISPC 受控机制调用外部验证服务（验证商品真实性）
//     - 通过 ISPC 受控机制调用外部估值服务（评估商品价值）
//     - 自动生成代币并上链
//
//  2. TransferCommodity - 商品转移
//     - 使用 token.Transfer() 转移商品份额
//     - 支持部分份额转移
//
//  3. EscrowCommodity - 商品托管
//     - 使用 market.Escrow() 创建商品托管
//     - 适用于商品交易、质押等场景
//
//  4. ReleaseYield - 收益释放
...
// CommodityContract 商品代币化合约
//
// 本合约展示如何使用 SDK 基础能力构建商品代币化应用：
//   - rwa.ValidateAndTokenize() - 商品验证和代币化（内部调用 token.Mint）
//   - token.Transfer() - 商品转移
//   - market.Escrow() - 商品托管
//   - market.Release() - 收益释放
//
...
func Initialize() uint32 {
	...
	event.AddStringField("contract", "Commodity")
...
// TokenizeCommodity 商品代币化
//
// 将现实世界商品（如黄金、石油、农产品等）转换为数字代币。
...
//
//	{
//	  "asset_id": "commodity_001",        // 商品ID（必填，如黄金编号）
//	  "total_supply": 1000000,            // 总供应量（必填，代表商品数量）
//	  "token_id": "COMMODITY_001"         // 代币ID（可选，如果不提供则由系统生成）
//	}
//
// 工作流程：
//  1. 解析参数并验证
...
func TokenizeAsset() uint32 {
	...
	// 步骤2：构建商品文档
	// 实际应用中，商品文档应包含完整的商品信息，如：
	//   - 商品类型（黄金、石油、农产品等）
	//   - 商品规格和数量
	//   - 存储位置和保管证明
	//   - 质量认证文件哈希
	//   - 所有权证明等
	documentsJSON := `{"asset_id":"` + assetID + `","type":"commodity"}`
	...
	//   - 链下系统同步
	//   - 审计和追溯
	event := framework.NewEvent("EquityTokenized")
	event.AddAddressField("owner", caller)
...
// TransferCommodity 商品转移
//
// 转移商品的代币份额。支持部分份额转移，适用于商品交易、投资等场景。
//
// 参数格式（JSON）:
//
//	{
...
func TransferAsset() uint32 {
	...
	// 步骤5：发出商品转移事件
	event := framework.NewEvent("CommodityTransferred")
...
// EscrowCommodity 商品托管
//
// 创建商品托管，将商品锁定在托管账户中，等待条件满足后释放。
// 适用于商品交易、质押、担保等场景。
//
// 参数格式（JSON）:
//
...
func EscrowAsset() uint32 {
	...
	// 步骤5：发出商品托管事件
	event := framework.NewEvent("CommodityEscrowed")
...
func ReleaseYield() uint32 {
	...
	// 步骤5：发出收益释放事件
	event := framework.NewEvent("YieldReleased")
	event.AddAddressField("from", caller)
