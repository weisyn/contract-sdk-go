...
// Package main 提供游戏道具NFT合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建游戏道具NFT合约。
// 通过本示例，您可以学习：
...
//
//  1. MintNFT - 铸造NFT
//     - 使用 token.Mint() 铸造唯一的游戏道具NFT
//     - 每个NFT都有唯一的tokenID和元数据（道具类型、属性、等级等）
//
//  2. TransferNFT - 转移NFT
...
//   - 每个tokenID只能有一个所有者
//   - NFT不可分割，转移时数量为1
type DigitalArtNFTContract struct {
...
func Initialize() uint32 {
	...
	event.AddStringField("contract", "GamingNFT")
...
// MintNFT 铸造游戏道具NFT
//
// 使用 helpers/token 模块的 Mint 函数铸造唯一的游戏道具NFT。
// 每个NFT都有唯一的tokenID和元数据（道具名称、类型、属性、等级、图片URL等）。
// SDK 内部会自动处理：
//   - 交易构建（自动构建 UTXO 交易）
//   - 事件发出（自动发出 Mint 事件）
//
...
//	  "token_id": "item_001",          // NFT的tokenID（必填，唯一标识）
//	  "name": "Legendary Sword",       // 道具名称（必填）
//	  "type": "weapon",                // 道具类型（必填，如weapon、armor、potion等）
//	  "level": 10,                     // 道具等级（可选）
//	  "attributes": "strength:100",    // 道具属性（可选）
//	  "description": "A powerful...", // 道具描述（可选）
...
func MintNFT() uint32 {
	...
	nameStr := params.ParseJSON("name")
	typeStr := params.ParseJSON("type")

	if toStr == "" || tokenIDStr == "" || nameStr == "" || typeStr == "" {
	...
	// 步骤5：发出NFT铸造事件（包含元数据）
	levelStr := params.ParseJSON("level")
	attributesStr := params.ParseJSON("attributes")
	...
	event.AddStringField("name", nameStr)
	event.AddStringField("type", typeStr)
	if levelStr != "" {
		event.AddStringField("level", levelStr)
	}
	if attributesStr != "" {
		event.AddStringField("attributes", attributesStr)
	}
	if descriptionStr != "" {
