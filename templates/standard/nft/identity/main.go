...
// Package main 提供身份证明NFT合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建身份证明NFT合约。
// 通过本示例，您可以学习：
...
//   - 每个tokenID只能有一个所有者
//   - NFT不可分割，转移时数量为1
type IdentityNFTContract struct {
...
//	  "identity_name": "Sunset Over Mountains", // 艺术品名称（必填）
...
//       "to": "<接收者地址>",
//       "token_id": "art_001",
//       "identity_name": "Sunset Over Mountains",
//       "issuer": "Alice"
//     }
//
//export MintNFT
func MintNFT() uint32 {
	...
	identity_nameStr := params.ParseJSON("identity_name")
	issuerStr := params.ParseJSON("issuer")

	if toStr == "" || tokenIDStr == "" || identity_nameStr == "" || issuerStr == "" {
	...
	event.AddStringField("identity_name", identity_nameStr)
