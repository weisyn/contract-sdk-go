...
// Package main 提供域名服务NFT合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建域名服务NFT合约。
// 通过本示例，您可以学习：
...
//   - 每个tokenID只能有一个所有者
//   - NFT不可分割，转移时数量为1
type DomainsNFTContract struct {
...
//	  "domain_name": "Sunset Over Mountains", // 艺术品名称（必填）
//	  "registrar": "Alice",               // 艺术家名称（必填）
...
//       "to": "<接收者地址>",
//       "token_id": "art_001",
//       "domain_name": "Sunset Over Mountains",
//       "registrar": "Alice"
//     }
//
//export MintNFT
func MintNFT() uint32 {
	...
	domain_nameStr := params.ParseJSON("domain_name")
	registrarStr := params.ParseJSON("registrar")

	if toStr == "" || tokenIDStr == "" || domain_nameStr == "" || registrarStr == "" {
	...
	event.AddStringField("domain_name", domain_nameStr)
	event.AddStringField("registrar", registrarStr)
	if descriptionStr != "" {
