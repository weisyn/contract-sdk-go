...
// Package main 提供证书凭证NFT合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建证书凭证NFT合约。
// 通过本示例，您可以学习：
...
//   - 每个tokenID只能有一个所有者
//   - NFT不可分割，转移时数量为1
type CertificatesNFTContract struct {
...
//	  "certificate_name": "Sunset Over Mountains", // 艺术品名称（必填）
//	  "issuer": "Alice",               // 艺术家名称（必填）
...
//       "to": "<接收者地址>",
//       "token_id": "art_001",
//       "certificate_name": "Sunset Over Mountains",
//       "issuer": "Alice"
//     }
//
//export MintNFT
func MintNFT() uint32 {
	...
	certificate_nameStr := params.ParseJSON("certificate_name")
	issuerStr := params.ParseJSON("issuer")

	if toStr == "" || tokenIDStr == "" || certificate_nameStr == "" || issuerStr == "" {
	...
	event.AddStringField("certificate_name", certificate_nameStr)
	event.AddStringField("issuer", issuerStr)
	if descriptionStr != "" {
