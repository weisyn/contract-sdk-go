...
// Package main 提供收藏品NFT合约示例
//
// 📋 示例说明
//
// 本示例展示如何使用 WES Contract SDK Go 构建收藏品NFT合约。
// 通过本示例，您可以学习：
...
// CollectiblesNFTContract 收藏品NFT合约
//
...
func Initialize() uint32 {
	...
	event.AddStringField("contract", "CollectiblesNFT")
...
// MintNFT 铸造收藏品NFT
//
// 使用 helpers/token 模块的 Mint 函数铸造唯一的收藏品NFT。
// 每个NFT都有唯一的tokenID和元数据（收藏品名称、系列、稀有度、图片URL等）。
// SDK 内部会自动处理：
//   - 交易构建（自动构建 UTXO 交易）
//   - 事件发出（自动发出 Mint 事件）
//
...
//	  "token_id": "collectible_001",  // NFT的tokenID（必填，唯一标识）
//	  "name": "Rare Card #001",       // 收藏品名称（必填）
//	  "series": "Legendary",          // 系列名称（必填）
//	  "rarity": "Ultra Rare",         // 稀有度（可选）
//	  "description": "A rare...",     // 收藏品描述（可选）
...
func MintNFT() uint32 {
	...
	seriesStr := params.ParseJSON("series")

	if toStr == "" || tokenIDStr == "" || nameStr == "" || seriesStr == "" {
	...
	// 步骤5：发出NFT铸造事件（包含元数据）
	rarityStr := params.ParseJSON("rarity")
	...
	event.AddStringField("series", seriesStr)
	if rarityStr != "" {
		event.AddStringField("rarity", rarityStr)
	}
	if descriptionStr != "" {
