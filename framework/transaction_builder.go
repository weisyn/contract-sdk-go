...
// ==================== WES 合约交易构建器（链式API）====================
//
// ⚠️ **内部接口**：仅供 helpers 层使用，外部开发者不应直接使用
//
...
//
// ⚠️ **内部接口**：仅供 helpers 层使用
func (tb *TransactionBuilder) AddInput(outpoint OutPoint, isReferenceOnly bool, unlockingProof UnlockingProof) *TransactionBuilder {
	if tb.err != nil {
		return tb
	}
	...
	if len(outpoint.TxHash) != 32 {
		tb.err = NewContractError(ERROR_INVALID_PARAMS, "txHash must be 32 bytes")
		return tb
	}
...
//
// ⚠️ **内部接口**：仅供 helpers 层使用
func (tb *TransactionBuilder) Stake(staker Address, amount Amount, validator Address) *TransactionBuilder {
	if tb.err != nil {
		return tb
	}
...
//
// ⚠️ **内部接口**：仅供 helpers 层使用
func (tb *TransactionBuilder) WithFee(feeAmount Amount) *TransactionBuilder {
	if tb.err != nil {
		return tb
	}
...
func (tb *TransactionBuilder) Finalize() (bool, []byte, uint32) {
	if tb.err != nil {
		return false, nil, ERROR_EXECUTION_FAILED
	}
	...
	// 调试：输出 Draft JSON，便于定位交易构建问题
	debugEvent := NewEvent("tx_draft_debug")
	debugEvent.AddStringField("json", draftJSON)
	_ = EmitEvent(debugEvent)

	// 调用宿主函数构建交易
	draftPtr, draftLen := AllocateString(draftJSON)
	if draftPtr == 0 {
		return false, nil, ERROR_EXECUTION_FAILED
	}
	...
	receiptBytes := GetBytes(receiptPtr, receiptSize)
...
func parseTxHashFromReceipt(receiptJSON string) ([]byte, uint32) {
	...
	if contains(receiptJSON, `"error"`) && !contains(receiptJSON, `"error":""`) && !contains(receiptJSON, `"error":null`) {
		return nil, ERROR_EXECUTION_FAILED
	}
	...
	if idx := indexOf(receiptJSON, `"unsigned_tx_hash":"`); idx >= 0 {
		...
		if end > start {
			...
			return hexDecode(hashHex), SUCCESS
		}
	}
...
func (tb *TransactionBuilder) serializeDraft() string {
	...
	for i, out := range tb.draft.outputs {
		...
		case "asset":
			json += `,"owner":"` + hexEncodeSimple(out.to) + `"`
			json += `,"amount":"` + Uint64ToString(out.amount) + `"`
			if len(out.tokenID) > 0 {
				json += `,"token_id":"` + hexEncodeSimple(out.tokenID) + `"`
			} else {
				json += `,"token_id":""`
			}
			json += `,"metadata":{}`
	...
	for i, intent := range tb.draft.intents {
		...
		json += `{"type":"` + intent.intentType + `","params":{`

		switch intent.intentType {
		case "transfer":
			json += `"from":"` + hexEncodeSimple(intent.from) + `"`
			json += `,"to":"` + hexEncodeSimple(intent.to) + `"`
			json += `,"token_id":"` + hexEncodeSimple(intent.tokenID) + `"`
			json += `,"amount":"` + Uint64ToString(intent.amount) + `"`

		case "stake":
			json += `"staker":"` + hexEncodeSimple(intent.from) + `"`
			json += `,"amount":"` + Uint64ToString(intent.amount) + `"`
			json += `,"validator":"` + hexEncodeSimple(intent.validator) + `"`
		}
...
func hexEncodeSimple(data []byte) string {
	if len(data) == 0 {
		return ""
	}
